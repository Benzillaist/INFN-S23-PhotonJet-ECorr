...
void photon_full_analysis_H(bool useArgs, char const *args) {
  ...
  // ntuple file name
  TString fileName = "photonGun_ntup10k_100-1000.root";
  auto treeName = "MyLCTuple";
  
  TString saveDir = "photonGun_10k_100-1000";
  ...
  TH1F *deltaR_0 = new TH1F("dR_0", "Delta R", 20, 0 , 0.002);
...
of true energy w.r.t reconstructed energy
  ...
  TH2F *KEtrue_0 = new TH2F("KEt_0", "Correction factor as function of energy", 100, lowE, highE, 100, 0.95, 0.97);
  ...
  if(ECA) {
    ...
    for(int i = 0; i < fitDivs; i++) {
      cout << corrEBins[i] << " " << corrEBinN[i] << endl;
      cout << corrTBins[i] << " " << corrTBinN[i] << endl;
    }
    ...
    Corr2DFitFunc_0->SetParameters(0.07, 0.2, 1.6, 0.89, 0, 0, 0);
    Corr2DFitFunc_1->SetParameters(-0.0065, 3.4, 0.92, 0, 0, 0);
    Corr2DFitFunc_2->SetParameters(0.07, 0.2, 1.6, 0.89, 0, 0, 0);
    ...
    TGraphErrors *CorrTermE_0 = new TGraphErrors(fitDivs, xE, corrEBins, exE, eyE);
    ...
    TGraphErrors *CorrTermT_0 = new TGraphErrors(fitDivs, xT, corrTBins, exT, eyT);
    ...
    CorrERes_Bins_0->GetXaxis()->SetTitle("Corrected Energy - True Energy (GeV)");
    ...
    CorrERes_Fit_0->GetXaxis()->SetTitle("Corrected Energy - True Energy (GeV)");
  ...
  if(ECF || rFH) {
	...
	strcat(outT, ".png");
      ...
      iGaussFit->SetParameter(0, 50);
      iGaussFit->SetParameter(1, 0.001);
      iGaussFit->SetParameter(2, 0.02);
  ...
  if(HCR) {
    ...
    // Prints out the bins used to find the weights
    for(int i = 0; i < fitDivs; i++) {
      cout << HCREBins[i] << " " << HCREBinN[i] << endl;
      cout << HCRTBins[i] << " " << HCRTBinN[i] << endl;
    }
    ...
    TGraphErrors *HCRatioE_0 = new TGraphErrors(fitDivs, xE, HCREBins, exE, eyE);
    ...
    TGraphErrors *HCRatioT_0 = new TGraphErrors(fitDivs, xT, HCRTBins, exT, eyT);
  ...
  if(pH) {
    ...
    K2Corr2D_0->SetMinimum(0.95);
    K2Corr2D_0->SetMaximum(0.97);
